...
const orbit = [['-42%', '-40%'], ['-28%', '-40%'], ['-14%', '-40%'], ['0%', '-40%'], ['14%', '-40%'], ['28%', '-40%'], ['42%', '-40%'], ['44%', '-24%'], ['44%', '-8%'], ['44%', '8%'], ['44%', '24%'], ['44%', '40%'], ['30%', '40%'], ['18%', '40%'], ['6%', '40%'], ['-6%', '40%'], ['-18%', '40%'], ['-30%', '40%'], ['-42%', '40%'], ['-44%', '24%'], ['-44%', '8%'], ['-44%', '-8%'], ['-44%', '-24%']].map(([x, y]) => ({ x, y }));
...
const renderDice = (dice) => list('#dice-row', dice, (die) => { const pipMarkup = dots[die.value] ? dots[die.value].map((position) => `<i class="pip ${position}"></i>`).join('') : `<b class="die-value">${escapeHtml(die.value)}</b>`; return `<div class="die ${die.frozen ? 'frozen' : ''} ${die.muted ? 'muted' : ''}" aria-label="${escapeHtml(die.value)}">${pipMarkup}</div>`; });
const renderTrack = (position) => { track.querySelectorAll('.track-cell').forEach((cell) => cell.remove()); orbit.forEach((point, index) => { const phase = index < 8 ? 'youth' : index < 15 ? 'midlife' : 'elder'; const current = index === position - 1; const cell = document.createElement('span'); cell.className = `track-cell ${phase} ${current ? 'current' : ''}`; cell.style.setProperty('--x', point.x); cell.style.setProperty('--y', point.y); cell.innerHTML = `<b>${index + 1}</b>${current ? '<em>AI</em>' : ''}`; track.append(cell); }); };
...
const renderDesk = (game) => {
  ...
  element('#phase-label').textContent = game.phase;
  ...
  renderTextList('#log-list', game.logs.map((text) => [text, '']), '暂无'); renderDice(game.dice); renderTrack(game.position);
  displayedResume = game.resume; list('#resume-cards', displayedResume, (card, index) => renderCard(card, { resume: true, index }));
};
const renderDemo = () => renderDesk({ phase: demoGame.phase, round: demoGame.round, position: demoGame.position, connection: '静态预览', opportunity: demoGame.opportunity, fate: demoGame.fate, goals: demoGame.goals, events: demoGame.events, debuff: demoGame.debuff, logs: demoGame.logs, dice: demoGame.dice, resume: demoGame.resume });
const snapshotToDesk = (snapshot) => {
  const stage = STAGE_LABELS[snapshot.stage] || snapshot.stage || '未开始';
  const position = snapshot.game_over ? snapshot.completed_turn : (snapshot.current_turn || snapshot.completed_turn);
